    ...
    <div className="v2">
      ...
      <nav className="v2-nav">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 28 }}>
          <a href="#projects" className="v2-nav-link">Projects</a>
          <div className="v2-contact-wrap" ref={contactRef}>
            <button className="v2-nav-cta" onClick={() => setIsContactOpen(o => !o)}>Contact ▾</button>
            {isContactOpen && (
          ...
          <div className="v2-contact-wrap" ref={langRef}>
            ...
            {isLangOpen && (
              <div className="v2-contact-dropdown">
                ...
                <Link to="/es" className="v2-contact-item" onClick={() => setIsLangOpen(false)}>
        ...
        <div className="v2-hero-inner">
          <div className="v2-hero-content">
            ...
            <motion.p className="v2-hero-tagline" initial="hidden" animate="visible" variants={fadeUp} custom={0.08}>
              Transforming complex problems into products ready for people.
            </motion.p>
            <motion.div className="v2-hero-chips" initial="hidden" animate="visible" variants={fadeUp} custom={0.14}>
              ...
              <span className="v2-chip">AI Driven</span>
      ...
      <section className="v2-statement">
        <div className="v2-statement-inner">
          <motion.blockquote className="v2-statement-quote" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
            "Design that doesn't serve those who truly matter is effort without purpose."
          </motion.blockquote>
      ...
      <section className="v2-projects" id="projects">
        <div className="v2-section-inner">
          <div className="v2-projects-header">
            <div>
              <motion.p className="v2-eyebrow" initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4 }}>Projects</motion.p>
              <motion.h2 className="v2-section-title" initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: 0.08 }}>Featured Projects</motion.h2>
            </div>
          </div>
          <div className="v2-filters">
            <motion.button className={`v2-pill ${activeTag === null ? 'active' : ''}`} onClick={() => setActiveTag(null)} whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} transition={{ duration: 0.15, ease: 'easeOut' }}>All</motion.button>
            {allTags.map(tag => (
                ...
                <motion.div key={project.title} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }} transition={{ type: 'tween', duration: 0.25, ease: 'easeOut' }} className={`v2-project-card ${project.link ? 'v2-project-card--linked' : ''}`} onClick={project.link ? () => navigate(project.link) : undefined}>
                  ...
                  <div className="v2-project-body">
                    <div className="v2-project-tags">
                      {project.wip
                        ? <span className="v2-project-tag v2-project-tag--wip">Coming soon</span>
          ...
          {hasMore && <div style={{ textAlign: 'center', marginTop: 48 }}><button className="v2-show-more" onClick={() => setShowAll(true)}>View all projects ({filtered.length})</button></div>}
          {showAll && filtered.length > VISIBLE_LIMIT && <div style={{ textAlign: 'center', marginTop: 48 }}><button className="v2-show-more" onClick={() => setShowAll(false)}>Show less</button></div>}
        </div>
      </section>
