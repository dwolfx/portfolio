        ...
        <div className="hero-inner">
          <div className="hero-content">
            ...
            <motion.div className="hero-chips" initial="hidden" animate="visible" variants={fadeUp} custom={0.14}>
              ...
              <span className="chip">AI Driven</span>
          ...
          <div className="experience-timeline">
            {[
              ...
              },
              {
                ...
                desc: 'A project-based engagement via a third party. I led the redesign of Autoline, a B2B vehicle financing tool for dealerships, simplifying complex registration forms without compromising banking compliance.'
              },
              {
                ...
                desc: 'I led the rebranding of the B2B merchant management platform. I introduced and standardized the Ant Design System, conducted research with merchants to eliminate operational bottlenecks, and collaborated with engineering for scalable adoption.'
              },
              {
                company: 'TradersClub',
                ...
                desc: 'Hybrid role (design + React code): I built the Design System for B2B and B2C and developed an automation system that reduced report creation time from weeks to just 15 minutes.'
              },
              {
                company: 'Rede Globo',
                role: 'Product Designer Jr, Front-end Specialist',
                start: 'Mar 2017',
                end: 'Sep 2020',
                desc: 'Where I transitioned into the field. I worked at the intersection of design and front-end for G1, GE, GShow, and Globo.com. I created the Parallax and FlyCarpet formats, adopted by Google as market standards, with a 20% increase in CTR via data-driven design.'
              },
            ].map((item, i, arr) => (
              <motion.div
                key={item.company}
      ...
      <section className="projects" id="projects">
        <div className="section-inner">
          <div className="projects-header">
            <div>
              <motion.p className="eyebrow" initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4 }}>Projects</motion.p>
              <motion.h2 className="section-title" initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: 0.08 }}>Featured Projects</motion.h2>
            </div>
          </div>
          <div className="filters">
            <motion.button className={`pill ${activeTag === null ? 'active' : ''}`} onClick={() => setActiveTag(null)} whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} transition={{ duration: 0.15, ease: 'easeOut' }}>All</motion.button>
            {allTags.map(tag => (
              ...
              {visible.map(project => (
                <motion.div key={project.title} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }} transition={{ type: 'tween', duration: 0.25, ease: 'easeOut' }} className={`project-card ${project.link ? 'project-card--linked' : ''}`} onClick={project.link ? () => navigate(project.link) : undefined}>
                  ...
                  <div className="project-body">
                    ...
                    {project.link && <span className="project-cta">View full case →</span>}
                  </div>
                </motion.div>
              ))}
            </AnimatePresence>
          </div>
          {hasMore && <div style={{ textAlign: 'center', marginTop: 48 }}><button className="show-more" onClick={() => setShowAll(true)}>View all projects ({filtered.length})</button></div>}
          {showAll && filtered.length > VISIBLE_LIMIT && <div style={{ textAlign: 'center', marginTop: 48 }}><button className="show-more" onClick={() => setShowAll(false)}>Show less</button></div>}
        </div>
      </section>
